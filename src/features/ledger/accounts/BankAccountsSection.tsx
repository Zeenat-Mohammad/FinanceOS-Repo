              ...
              <tbody>
                {pageRows.map((account) => (
                  <tr key={account.id} className="border-t border-border/60">
                    ...
                    <td className="px-4 py-2 text-muted">{new Date(account.updated_at).toLocaleDateString()}</td>
                    <td className="px-4 py-2">
                      <div className="flex flex-wrap gap-1">
                        <Button type="button" className="h-7 border border-border bg-transparent px-2 text-xs" onClick={() => onEdit(account.id, account.name)}>
                          Edit
                        </Button>
                        <Button type="button" className="h-7 border border-border bg-transparent px-2 text-xs" onClick={() => onArchive(account.id)}>
                          Archive
                        </Button>
